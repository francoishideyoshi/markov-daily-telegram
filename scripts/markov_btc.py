...
def fit_hmm(close: pd.Series):
    ...
    labels_hmm = ["Bear", "Sideways", "Bull"]
    return [(labels_hmm[rank], int(k), float(means[k])) for rank, k in enumerate(order)]
...
def build_message(close: pd.Series, labels: pd.Series, P: np.ndarray,
    ...
    nd       = P[cs]        # [Bear, Side, Bull]
    d2       = P2[cs]
    d3       = P3[cs]

    signal    = float(nd[2] - nd[0])
    direction = ("\U0001f7e2 LONG" if signal >= 0.3 else
                 "\U0001f534 SHORT" if signal <= -0.3 else "\U0001f7e1 NEUTRAL")
    ...
    if hmm_regimes:
        for label, k, m in hmm_regimes:
            emoji = "\U0001f534" if label == "Bear" else ("\U0001f7e1" if label == "Sideways" else "\U0001f7e2")
            lines.append(f"\u2022 {emoji} {label}: `{m*100:+.3f}%` mean daily return")
    else:
